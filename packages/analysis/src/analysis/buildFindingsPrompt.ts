...
const CHAIN_LEGENDS: Record<string, string> = {
  ...
  "healing-gap": `- "healing-gap": the healer produced no heals or casts for facts.durationS seconds (facts.freeS of that was free of CC — time they COULD have cast), while facts.pressured (facts.pressuredSpec) took real damage and a teammate sat at facts.lowestAllyHp% HP during the gap — that crisis HP is why this gap was reported, not the gap length by itself. Coach healing rotation/triage awareness during that stretch.`,
