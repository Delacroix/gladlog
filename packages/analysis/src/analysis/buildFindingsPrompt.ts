...
const CHAIN_LEGENDS: Record<string, string> = {
  ...
  "crisis-no-response": `- "crisis-no-response": at facts.t the player's own HP fell to facts.hpPct% while taking facts.dmg2sPct% of max HP in the prior 2 s from facts.attackers attacker(s) (enemy burst cooldown active: facts.burst), and for the next 3 seconds did NOTHING to answer it — no self-heal ≥15%, no personal wall, no external, no CC/root/interrupt on an enemy, no kiting. The player was free (not CC'd, not locked out, alive through the window). Outcome reference from the match corpus for this bracket and damage level (cell facts.cellKey; fellBack=yes means a coarser bracket-wide cell): healers who did NOT respond within 3 s died within 10 s facts.refDeathNoResp% of the time (n=facts.refNNoResp); healers who DID respond died facts.refDeathResp% of the time (n=facts.refNResp). Among healers who DID respond here, the most common answers were facts.refTop. These are descriptive contrasts, not causal proof — responding players may have been in easier spots. CITE the numbers; do NOT turn them into "you should have pressed <ability>".`,
};
