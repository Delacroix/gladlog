...
export const MITIGATION_VERDICTS: Record<string, IMitigationVerdict> = {
  "31821": {
    ...
    source:
      ...
      "取 never 而不是 kill-live-gated,是因为本册自己的先例按**形态**而不是按数值分档:同为 20% " +
      "的真言术:障(62618,团队区域减伤)判 never,而 20% 的圣佑术/树皮术/壮胆酒(个人墙)判 " +
      "kill-live-gated,20% 的铁木树皮(指向队友的外放)甚至判 unconditional。光掌是团队区域减伤," +
      "与真言术:障同形态。若原意是「一般可以但也有例外」,那就是 kill-live-gated,改一个词即可 —— " +
      "两者的差别是:never 永不产出「打进减伤」的浪费判断,kill-live-gated 只在当时本来也打不死时产出。",
