...
describe("extractMatchDynamics", () => {
  ...
  it("calculates CC per minute correctly when events exist (B53)", () => {
    const ccOnFriend = makeAuraEvent(
    ...
    // NOTE: #24 fix — analyzeOutgoingCCChains' target filter is now id-set
    // membership in the passed second-arg collection, not `reaction`, so the
    // swap call (enemies, friends) works regardless of each unit's reaction.
    // f1 keeps its default Friendly reaction; only e1 is actually Hostile.
    const f1 = makeUnit("f1", { auraEvents: [ccOnFriend] });
    const e1 = makeUnit("e1", {
      reaction: CombatUnitReaction.Hostile,
      auraEvents: [ccOnEnemy],
    });
