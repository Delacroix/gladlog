/**
 * absorbsIn semantics probe (coaching-grounding-audit D7): GladUnit.absorbsIn is keyed
 * by the ATTACKER (damage I dealt that a shield soaked), not the victim. Measures how far
 * "absorbsIn as absorbs-taken" is from the real absorbs-taken (victim = decodeAbsorbed().victimGuid)
 * and how often the most-damaged-friendly pick flips. Usage: npx tsx packages/eval/scripts/absorbSemantics.ts <logDir> [maxRounds]
 */
...
const snapshots: Map<string, number>[] = []; let curTaken = new Map<string, number>();
function handle(m: any) {
  const taken = snapshots.shift() ?? new Map<string, number>();
  if (n >= maxM) return; n++;
...
(async () => {
  for (const f of files) { if (n >= maxM) break; const rl = readline.createInterface({ input: fs.createReadStream(f), crlfDelay: Infinity }); for await (const line of rl) { const pl = parseLine(line); if (pl?.arenaStart) curTaken = new Map(); else if (pl?.absorbed && Number.isFinite(pl.absorbed.absorbedAmount)) curTaken.set(pl.absorbed.victimGuid, (curTaken.get(pl.absorbed.victimGuid) ?? 0) + pl.absorbed.absorbedAmount); else if (pl?.arenaEnd) snapshots.push(curTaken); p.push(line); if (n >= maxM) break; } p.end(); }
  ...
  console.log(`top-damaged-friendly (dmgIn+absorbs) differs wrong-vs-right in ${flipTop}/${n} rounds`);
  console.log(`grounding totem units=${totemUnits} withAbsorbsIn=${totemWithIn} withAbsorbsOut=${totemWithOut}`);
})();
