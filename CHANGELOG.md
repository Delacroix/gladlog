...
来源:死亡回顾可视化升级 + 一处交互修缮 + 测试覆盖专项(六任务)。

### 回放 / 战报

- `3d52ce2` `60a541a` 死亡回顾升级为双栏:左侧事件表的伤害数字标红、治疗标绿;
  右侧新增死前 10 秒血量曲线——掉血段红、回血段绿、持平灰,控制(金)与
  防御技(绿)在曲线上有竖线标记,悬停显示技能名,终点标 ☠。旧日志无高级
  数据时维持原单栏样式。
...
- `fec622d` `2dee06f` 视觉基线随死亡回顾双栏重录(已人审);E2E 导航定位改
  精确匹配,修掉与回顾卡按钮子串撞名的竞态。
- `8fb4869` `eb7c43c` `ab2f1d7` `f56e6b6` 文档:覆盖提升计划收官回填、死亡
  回顾设计定稿与实现计划、CHANGELOG 基建与 v0.0.1–v0.1.8 追溯补记。
