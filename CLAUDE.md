...
**Where existing predicates live: see [`docs/predicate-index.md`](docs/predicate-index.md)** (116 entries plus a 13-row feature-flag ledger, with a consistency test in `packages/eval/test/predicateIndex.test.ts`: renaming/moving a symbol turns CI red). The index doesn't only track analysis↔gate pairs: since 2026-08-04 it also tracks cases within the desktop renderer where **two consumers** check the same fact (the "Report UI" section), with the same criterion of "one fact, one predicate". Check the table before writing new code — the rule has never been lacking; what was lacking was the index: on 2026-08-01 someone read this section and still hand-copied two predicates that same day. When the index went live, it immediately caught 5 registered violations, **all closed on the same day** (4 converted to shared exports, 1 confirmed to not be a duplicate); register newly discovered duplicates in the doc's "Not yet unified" section. That section is **not** empty and is not supposed to be: as of 2026-08-23 it carries one open divergence (`uwcCorpusScan` reads `DR_CATEGORIES_GENERATED` directly while the product goes through `DR_CATEGORY_MAP`'s override layer — they disagree on 33786 Cyclone and 99 Incapacitating Roar) plus two entries recorded as *deliberately* not shared, so nobody "unifies" them by mistake. Read it before concluding a duplicate is new.
...
- **2026-08-13**: the list need not gate the *lookup* to do this — it can gate the **verdict**. `getDispelType` is clean (official DB2, hand fallback emptied 2026-07-25), but `dispelAnalysis.getPriority` recognises a buff only via two hand lists (the mitigation allow-list → Critical, `spellCategories` 94 entries → High/Medium); anything unlisted is Low, and the missed-dispel analysis only collects Critical/High. **Not listed = permanently invisible**, and it looks exactly like "there was nothing worth dispelling". Measured: 72 corpus buffs that are officially dispellable were being scored Low — Ice Barrier is the clean example (official Magic type, simply no category entry).
...
**2. Attribution-shaped work is written backwards.** Hand-write the target conclusion sentence first ("if this were good, what would it say about this real match?"), get it approved, *then* build the smallest engine that produces it. Do not build the pipeline first and hope intelligence appears at the end.
