...
**现有谓词在哪,查 [`docs/predicate-index.md`](docs/predicate-index.md)**(54 条,配 `packages/eval/test/predicateIndex.test.ts` 的一致性测试:符号改名/移动会打红 CI)。写新代码前先查表 —— 这条规则不缺,缺的一直是索引:2026-08-01 有人读过本节、当天仍手抄了两处谓词。该索引上线时当场查出 5 处在册违规,列在文档的「尚未统一」一节。
