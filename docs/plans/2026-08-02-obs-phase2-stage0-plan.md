...
本计划覆盖设计文档 **§4 第 0 段**(五项中的四项 —— `CaptureBackend` 已挪到第 1 段,见下面 Task 6 的说明)+ **§9.4 自检表的门测子集**。第 1 段(托管 OBS
实例)另出计划,**要等门测结果回来再写**。
...
### Task 6:(已挪到第 1 段,2026-08-03 用户拍板)

原计划在第 0 段建 `captureBackend.ts` + `obsWebsocketBackend.ts` 并单测,**但不接线**。

开工前的冲突扫描发现它与复核规则的 YAGNI 一条正面冲突:接线被取消之后,它就是一个
**零消费者的抽象**,任务复核几乎必然标成投机性抽象。而 spec §4.4 当初把它列进五项地基的
理由是「让接口被一个真实现验证过」—— 那个理由在不接线之后已经不成立。

**裁决:整体挪到第 1 段**,与托管 backend、旁路 backend 两个实现一起写 —— 那时接口才有
真实约束,形状才定得住。设计文档 §4.4 已同步改。

第 0 段因此是 **6 个 Task**:1、2、3、4、5、7。
