...
## 判定谓词规格(来自 arenacoach 公开目录)

> **2026-08-02 更正**:本节标题原写「阈值为其 25 万场调参值」,**是误读**。
> arenacoach 的 mistakes 页面上 250,067 是**滚动 30 天的分析场次量**(与「3,281,474 条
> 检出」「21 条活跃规则」并列的头部统计),不是调参语料;没有任何一手资料声称阈值是在
> 25 万场上调出来的。
>
> 另需注意:21 条规则里**只有 5 条公布了具体数值**(cc-004、defensive-001、
> defensive-002、offensive-002、offensive-003),其余 16 条只有散文描述 ——
> **包括本批抄的 trinket-001**,它的「中立局面」没有任何公布的数值阈值。
> 所以下表「关键阈值」一列里,凡不在那 5 条之列的数字都是 gladlog 自己定的,
> 不是 arenacoach 的值。
>
> 规则目录全景(21 条 × 严重度 × 30 天检出数 × 每场发生率)见 `docs/BACKLOG.md` #18。
