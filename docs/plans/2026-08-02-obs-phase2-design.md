...
| **U2** | 默认码率与分辨率。影响盘与 CPU。**倾向 1080p60 / 8Mbps**(80GB 配额下约 22 小时素材),硬件编码可用时上调。**存疑**:arenacoach 自称一场 5–20MB,折算约 0.5 Mbps,与此差两个数量级(§2.12)—— 不采信,但说明「回放复盘够用」的码率可能远低于直觉。由 §9.4 的 bitrate 列实测后定 |
...
| **U6** | `PRE_ROLL_S`(§4.1 点击事件的前置回滚秒数)。arenacoach 用 **3 秒**。倾向直接取 3,但 gladlog 的事件粒度更细(finding / 爆发窗 / 死亡链),可能需要按事件类型分档                                                                                                            |
