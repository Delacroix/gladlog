...
**整条路线的生死项,也是调查里唯一没法从源码判定的**:

> OBS 最小化到托盘、没有可见窗口时,`game_capture` 能不能钩上 WoW?

钩取理论上不依赖 OBS 窗口焦点,但**没有任何源码或文档能证实**。这一步必须排在
第 1 段的**最前面**,在写任何托管逻辑之前:手动解压一份 OBS 到临时目录、手写一份
最小配置、用 §5.4 的命令行拉起、开 WoW、看录出来的画面是不是黑的。

同时验掉三个**推断**:便携路径的 cwd 依赖(§2.3.5)、删 sentinel 能否压掉崩溃框
(§2.3.2)、`tar -xf` 在 Windows 上解 zip(§2.11)。

**钩不上 → 整条路线要重估**(退路见附录 A)。这一步花十几分钟,能挡掉数周返工。
...
| 编号   | 事项                                                                                                                                     |
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **U1** | 空闲期分片间隔(§5.5)。太短则分片碎、索引行多;太长则挂机段占盘。**倾向 10 分钟**,自检实测后定                                             |
| **U2** | 默认码率与分辨率。影响盘与 CPU。**倾向 1080p60 / 8Mbps**(80GB 配额下约 22 小时素材),硬件编码可用时上调                                   |
| **U3** | 要不要用 `CreateRecordChapter` 往文件里插"开场 / 结束"章节标记。纯增强,只有 hybrid_mp4 支持(正是我们的容器)。**倾向做,成本极低**         |
| **U4** | 音频轨。**倾向:默认只录桌面音,不录麦克风**(隐私默认关);麦克风做成开关                                                                    |
| **U5** | OBS 版本升级策略。钉死 32.2.1 会过期。**倾向:钉死 + gladlog 发版时手动升,不做自动更新**(`--disable-updater` 已经关掉了 OBS 自己的更新器) |

### 风险

| 风险                                                                      | 应对                                                                  |
| ------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| **托盘态下 game_capture 钩不上 WoW** —— 全案生死项,源码无法判定           | §3 的门,第 1 段第一步实测                                             |
| 便携路径的 cwd 依赖是**推断**                                             | 同上,一并实测                                                         |
| 无人值守启动被模态框卡死(崩溃哨兵 / 首次向导)                             | §5.2 删 sentinel + 预写 FirstRun;自检命令里"有没有弹框"是独立一列     |
