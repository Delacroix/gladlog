...
### 2026-08-04 修订(三处偏离,已获用户确认)

真机验证与第 1 段实施过程中,以下三处偏离了本节原拍板,已于 2026-08-04 逐条征得
用户确认(流程与出处见 `docs/plans/2026-08-04-obs-phase2-stage1-plan.md` 头部):

1. **下载改为用户可见动作**,不是"首次运行时自动下载"——设置页给一个显式按钮 +
   进度条(见 §5.1 修订)。
2. **非 win32 保留旁路能力**,不强制 `recordingEnabled=false`——mac/linux 用户仍可
   用"使用我自己的 OBS"(见 §8 修订)。
3. **编码器第 1 段钉死 `obs_x264`**,NVENC 挂第 2 段——websocket 没有编码器枚举
   API(§2.5 早已写明),原 §5.3 的"运行时探测切换"与之矛盾(见 §5.3 修订)。
...
### 门测结果:2026-08-04 真机已过

**通过。** 用 §9.4 的自检命令在真机上跑通,关键结果:游戏采集出画面(4 号风险项
排除,截图不黑)、录像可播、便携模式生效(cwd/profile 假设成立)、
`--websocket_ipv4_only` 后 IPv4 连通(真机上 websocket 只绑了 IPv6,是 §2.5 两种
命令行写法之外又一处坑)、`obs_x264` 编码器可用、录制/分片/停止全链路走通。

真机教训:就绪判定必须读 OBS 自己的日志而非裸 TCP 探测(且要防上一轮旧日志假阳性
—— 见 §5.4 实施);websocket 没有编码器枚举 API,第 1 段因此钉死 `obs_x264`
(见 §5.3 修订)。spawn 谜题尚未定位根因,留给 Task 3 的真机实验逐变量排查。
...
### 5.1 获取:用户可见动作触发下载(2026-08-04 修订)

> **偏离原拍板(§0 原写"首次运行时自动下载"),已获用户确认**:定稿改为**用户可见
> 的下载动作**——设置页一个按钮触发,进度条显示已下载 MB / 总量,失败可重试。理由:
> 179MB 的静默后台下载对毫无察觉的用户不友好,首次下载失败时"到底发生了什么"必须
> 可见才能自助排障。
...
### 5.3 编码器选择:第 1 段钉死 obs_x264,NVENC 挂第 2 段(2026-08-04 修订)

> **原文与 §2.5 自相矛盾,已修正**:原文写"用 websocket 枚举可用编码器",但 §2.5
> 已经写明**websocket 没有任何编码器枚举 API**——2026-08-04 真机复核确认(日志里的
> "Available Encoders"为空来自 OBS 自己的日志,不是一次 websocket 请求能拿到的)。
> 运行时探测再切换的设计前提不成立,已获用户确认改为下述做法。

`AdvOut/RecEncoder` 的合法 id 是硬件相关的(`obs_x264` / `jim_nvenc` / `obs_qsv11` /
`h264_texture_amf` …)。第 1 段**钉死 `obs_x264`**(软件编码,人人都有,不需要探测):
写进 `recordEncoder.json`,**不做运行时切换**(编码器选择要输出处理器重建,也改不了)。
**NVENC(或其它硬件编码器)的选择逻辑挂第 2 段**——需要另找一条不依赖 websocket 枚举
的路径(候选:解析 OBS 日志里的"Available Encoders"行,或探测显卡型号后按白名单
猜测),Task 3 真机日志尾已留下证据,留给第 2 段设计。
...
- **缺头天然解决**:分片是在上一场结束或空闲时切的,它的起点必然**早于**下一场开场,
  所以 `headroom = source.startTime − chunk.startedAt > 0` 恒成立。已知两类例外,
  **均如实记录,不许静默**:
  1. WoW 刚启动就直接进场(`StartRecord` 与开场几乎同时),headroom 可能为负;
  2. **两场间隔小于日志滞后时**(复核 I16,第 1 段实施发现):`segmentClose` 触发
     `splitChunk()` 依赖日志到账,若下一场紧跟着开场且日志滞后把 `segmentClose`
     推迟到了下一场开场之后,切刀时机就会晚于下一场真实开场,同样产生负 headroom。
     产品侧在 `associate()` 命中时用 `log.warn` 记两个时刻(见 Task 5 实施);
     §9.1 的验收基线脚本(`scripts/headroomBaseline.ts`)按同一公式统计,不单独
     排除这类样本。
...
| 安装包体积    | **不变**                                                                                                                                                                                                                                        |
| 平台守卫      | 托管模式仅 win32,非 win32 上托管相关设置项禁用并说明。**2026-08-04 修订(已获用户确认,偏离原文的"`recordingEnabled` 强制 false")**:非 win32 **保留旁路能力**——"使用我自己的 OBS"(§5.7)在 mac/linux 上仍可开,只是托管进程/下载/分片这条路径不可用 |
| 死配置        | `packages/desktop/electron-builder.yml`(`files` 白名单会丢 node_modules,是颗独立于本路线的雷)**已删除**(2026-08-05,Task 7);`npmRebuild`/`files` 留在 `package.json` build 块的默认值,见 `docs/BUILD-WINDOWS.md`                                 |
