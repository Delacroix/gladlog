...
| prompt MATCH SUMMARY 头行渲染出的 `Result: Win/Loss` 标签 | `packages/eval/src/halo/redactOutcome.ts` → `RESULT_LABEL_RE`                        | `redactOutcomeLabels`(光环实验涂抹);`buildHaloArms`(index 与 prompt 交叉核对,不一致即 throw) | 锚定 `buildMatchContext.ts:802` 的内联模板——该行没有单独导出的格式化函数可 import,属「门规重新解析渲染文本」一类,不是共享常量。一致性由 `outcomeHalo.test.ts` 的头行夹具与 `buildHaloArms` 的运行时交叉核对把守,而非编译期断言相等。 |
