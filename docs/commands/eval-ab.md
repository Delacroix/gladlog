...
5. **盲评:**
   ...
   **K 重判官(可选,默认 K=1):** 2026-08-06 验收实测(spec
   `2026-08-05-judge-noise-floor-design.md` 验收结果)K=3 中位数只把同内容配对 SD 从
   0.93 降到 0.75——副本误差相关,未过 ≤0.5 硬线,**不作为默认**;A/B 默认单判官
   `blind/scores/ITEMID.json`,最小可检 |Δ| 维持 ~0.4,更小的差异以确定性文本判据裁决。
   如实验性启用 K 重:每个 ITEMID 派 K 个独立判官(同件判官互不知情,一件一代理铁律
   不变),分数分别写 `blind/scores/ITEMID.r1.json` / `.r2.json` / `.r3.json`。
   abStats 自动识别两种命名:K 模式先对每件逐维取中位数再配对;某件不足 2 份按缺分
   整对丢弃,恰 2 份取均值并在 replicateSummary 里标注。accuracy 一律以各判官
   factAudit 的 computeAccuracyFromFactAudit 计算值参与聚合(rubric 见 eval-baseline.md)。
