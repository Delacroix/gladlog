...
**量化(先测量,`packages/eval/scripts/auraDoubleCloseScan.ts` + `src/explore/auraDoubleClose.ts`,
全库 1028 场,0 错误)**:该诊断脚本独立重放 `buildAuraIntervals` 的开闭配对逻辑(不改动生产
代码),对每次"关闭事件找不到开区间"的回退分支触发,额外记录"距同 spellId 上一次关闭事件
的间隔"这一个生产函数本身不算的信号。全库结果——回退分支总触发 96089 次,其中 32384 次
确无任何在先关闭(真·"赛前已挂,只见掉落"合法情形,不受本次修复影响);其余 63705 次有在先
关闭,按间隔分布(累计):≤0.01s 45719、≤0.1s 53421、≤0.5s 61620、≤1s 63590、≤2s 63613、
≤5s 63638、≤10s 63673、≤30s 63686——**间隔集中在亚秒级**(≤0.5s 已占非空间隔的 96.7%,
≤1s 占 99.8%),往上几乎不再增长(1s→30s 仅 +96),证明"冗余关闭事件重复上报同一次真实
掉落"和"真的相隔很久的独立掉落"在间隔尺度上界限分明,不是拍脑袋。

按 1 秒阈值(`DUPLICATE_CLOSE_WINDOW_S`,取值见下)分类:**幻影区间 63590 处,波及 1023/1028
场(99.5%)**。发生率如此之高的原因是机制本身很常见——绝大多数硬控(冰冻术/闷棍/变形术/
心灵尖啸等)掉落时,WoW 战斗日志经常连续打出 `SPELL_AURA_BROKEN`/`BROKEN_SPELL`/`REMOVED`
中的不止一条,而 76ea5f90 只是审查者最先撞见的一例。

**机制**:关闭事件到达且 `open` map 里找不到该 spellId 的任何开区间时,原代码无条件判定
"赛前已挂,这场只看到它掉落",按官方时长回推伪造区间。修法:改判"这个 spellId 距上一次
已发出的关闭事件(不论来自正常配对还是更早的回退分支)是否在 `DUPLICATE_CLOSE_WINDOW_S`
(=1 秒)内"——命中则视为同一次真实掉落的冗余关闭事件上报,丢弃、不产出区间;未命中才走
原回退分支。改动只影响这一条判定路径(`auraIntervals.ts:118-172`),不动正常配对、DOSE
语义、或"精确 key 优先、同 spellId 回退"的关闭策略。TDD 覆盖(`test/ported/auraIntervals.test.ts`
新增 4 例):精确复现 76ea5f90 的双关闭 1ms 竞态(只出一段)、三重冗余关闭事件挤在一起(仍只
出一段)、以及两个负控制(真·赛前已挂的孤立 REMOVED 不受影响;同一 spellId 两次相隔 60 秒
的独立掉落,第二次仍照常回推,不被误吞)。

**前后数字(同判据)**:76ea5f90 @173s,`aurasActiveAt` 渲染"冰冻陷阱、冰冻陷阱"(重复)→
修复后仅"冰冻陷阱"(单条),两处额外抽样复核(c84e13b5 的 Eranu 冰冻术多重 `BROKEN_SPELL`
链、d2a90ac4 的 Холод)均未见重复名。诊断脚本量出的判据(间隔 ≤1s 的回退分支触发数)
**63590 → 0**——阈值判定逻辑就是现在生产代码里跑的那一份(非重新推导),故这不是"读代码 +
可信的 commit message",是全库计数验证过的判据翻转。

**范围内确认无回归**:`packages/analysis` 全量测试(含 `momentSnapshot.test.ts`、
`counterfactual.test.ts`)、`packages/desktop` 全量测试(含 `report.aurauptime.test.tsx`)均绿;
`npm run typecheck`、`npx eslint . --quiet` 干净。

**谓词索引核对**:`docs/predicate-index.md`「尚未统一」一节 2026-08-05 登记的
`utils/utils.ts` 与 `utils/auraIntervals.ts` 撞名条目,判定与本条无关(见上方"影响面"段落的
既有结论)——本次修复不涉及那条撞名登记,未改动谓词索引。
