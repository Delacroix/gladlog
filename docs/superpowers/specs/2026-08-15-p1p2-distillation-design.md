...
1. **P1/P2 主评估**:eval A/B(`/eval-ab` 流程,prompt-builder 变更的受控盲评)——加四类型前 vs 后。
2. **约束预算审计 rider**(用户 2026-08-14 之问):同一 A/B 流水线加第三臂「选择性放松」(实验性关闭若干输出空间类约束,候选清单在计划里逐个列名),三臂同判:**验真新发现率 vs 机制错误率**(判据用金标集校准口径)。产出帕累托数据表,裁决权在用户——本 spec 不预设结论。
