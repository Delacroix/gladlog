...
| calibration 7/7 不倒退  | 未跑(等 K=3 采纳与否拍板)                                                          | —                                                                                   |
| accuracy 契约一致性     | 300/300 零失配(accuracyMismatches=0)                                               | ✅ 确定性 accuracy 可独立采纳                                                       |

详细报告:eval-home `ab/2026-08-06-planted-accuracy/report.md`。结论:确定性 accuracy(Task 1-2)成立;K=3(Task 3-4 协议)未过硬线,采纳与否及替代方向(异构副本/上调最小可检 Δ)待用户决策。
