...
- **参照不用分段界定,用结果**:用户:「不要用分数界定」。结论句改为:
  ...
  > 这种状态下没应对的治疗 **22%** 在 10 秒内死亡,出手的只有 **8%**(n=…);高手最常见的应对是自疗 75%、减伤 23%、外置 14%。

  参照表每格存:`nNoResp / death10NoResp`(全体玩家,无应对)、`nResp / death10Resp`(全体玩家,有应对)、`top`(仅 top10 的应对分布,分段只在这里用)。
