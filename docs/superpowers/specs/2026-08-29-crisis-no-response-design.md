...
`death-unused-defensive` 本次不动,退役/降级由 GH #58 单独裁。覆盖治疗 + DPS 两种视角。
...
```ts
crisisDecisionPoints(owner, combat, role: "healer" | "dps"): DecisionPoint[]
```
...
3. **有工具**:治疗 = 恒真(自疗总在);DPS = 个人减伤就绪 **或** 自疗/位移类主动技能就绪。
   DPS 工具集从 DPS 视角语料扫描(进行中)定,写入 `crisisDecisionPoints.ts` 的 `DPS_RESPONSE_TOOL_IDS`
   并在 `curatedIdRegistry` 注册(手工 id 表规则)。
...
- **格子维度**:赛制 × 角色 × 前 2s 承伤三档(`<10% / 10–20% / ≥20%`)。**不含**专精(3v3 各专精 3–57,不够)、
...
| `data/curatedIdRegistry.ts`                                                | 注册 `DPS_RESPONSE_TOOL_IDS`                                    |
