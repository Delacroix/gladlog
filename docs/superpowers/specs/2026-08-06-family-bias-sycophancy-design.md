...
| 判据      | 实测                                                                                                                                                                                                                         | 判定        |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| D1 完整性 | 200/200(S 100 + D 100),matchId 零失配;臂↔族实检 3/3                                                                                                                                                                          | ✅          |
| D1 双差分 | **focusCalibration familyBias +0.84 [+0.56, +1.12] 同族偏差确凿**;accuracy +0.40 [−0.04, +0.82] 踩线;其余回复维含零;prompt 维非零(sufficiency −0.42/scaffolding −0.26)判定为回复→prompt 光环伪影(两臂 prompt 逐字节同)副发现 | ✅ 如实报告 |
| D1 严宽度 | accuracy S−D = −1.18,归因 factAudit 勤奋度:S 逐件驳倒 2.65 vs D 1.26(确定性契约使其可见)                                                                                                                                     | ✅ 分离报告 |
| D2 有效性 | 30/30 挑战(≥10 场),教练回复 30/30                                                                                                                                                                                            | ✅          |
| D2 指标   | **缴械率 0%、含糊率 0%(双族判官一致 30/30,一致率 100%)**——sonnet 教练全部重申日志事实;>30% 修法线远未触发,基线留作回归套件(定种子可复现)                                                                                     | ✅          |

报告:eval-home `ab/2026-08-06-family-bias/report.md` 与 `ab/2026-08-06-sycophancy/report.md`。口径含义:跨族 A/B 的 focusCalibration 维需双族判官均值或降权(后续任务);DeepSeek 判官事实审计强度弱于 sonnet,当中立参照需打折扣。
