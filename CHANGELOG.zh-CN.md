...
## v0.1.20-obs2.1(2026-08-03,测试包)

OBS 录像二期**第 0 段**的功能测试预发布,来自 `worktree-obs-phase2` 分支,未并入 main。第 0 段是与「最终用哪个采集端」无关的那部分;托管 OBS 本身(第 1 段)不在这个包里,而且要等下面那个门测脚本在真机上跑过之后才会开始规划。本包同时带上 v0.1.19 之后并入 main 的全部改动。

### 录像回放(修掉一个已发布的 bug)

- `7efe296` `f6a80b2` `cbc4646` **在录像 tab 里点死亡、finding 或爆发窗,以前跳过去总是晚几秒** —— 每次都晚,晚的量正好是战斗日志的整个滞后(好的时候 2 秒,差的时候几十秒)。录像总是在开场之后才起,而表达这个差值的偏移量被压成了零,于是画面系统性地落在你点的那个事件之后。现在是准的。
- `e9e1555` 点标记条上的标记,行为和在列表里点同一时刻一致了 —— 两者都回滚 **3 秒**再开始播,让你看到起手而不是结果。
- `f6a80b2` 录像晚于开场时,缺掉的秒数会明写在 tab 上(「缺头 N 秒」)而不是把所有东西静默平移;落在那段空档里的时刻会标灰并给出原因;播放器解不开的录像会直说,而不是给一块黑屏。

### 录像存储

- `ea2a63b` `1e228b1` 一段录像现在可以对应多场对局。以前背靠背两场共用一段录像时,第二场静默地什么都拿不到。
- `92f9e24` `d47b5d8` 保留策略在原有的数量上限之外加了**体积上限**(默认 80GB),而且在录制失败之后与启动时也会执行 —— 以前连着失败几次就一次都不回收。配置被改坏或手工写错也不会再被读成「全删」:两个闸都是失效安全的。

### 录像状态

- `88fdfc1` `7841af7` 录制状态终于露面了:设置页里一行实时状态,以及「已启用但没连上」时主界面的一条横幅 —— 这个组合以前是完全静默的,你只能在发现没有录像时才知道。

### 工具

- `db3e721` `c3e00e7` 新增 `npm run recorder:gatecheck --workspace=packages/desktop` —— 在 Windows 上跑一发的探针:自动下载并配置一份便携版 OBS、无头拉起,然后用一张表报出显卡是否一致、提权是否一致、有没有冲突的覆盖层钩子、游戏采集出的画面是不是黑的、分片能不能用、以及实测码率。它的输出决定第 1 段怎么做。

### 来自 main

- `959c1f6` 同水平分析板块不再「有时候不出现」。
- `2a1222d` `476eb4c` `15b8ed1` 录像 tab 布局:时刻清单吃满侧卡高度(2560x1440 下浪费的 653px 降到 11px),视口预算收敛为一处。
- `b4c0345` `5fdfff7` `105f0ec` `0d49b4c` `57720ac` 开发者页改版为 DevTools 式工作台。

### 其他

- `017b375` `1076181` `aa7ff48` `a69d268` `d5b483b` `a0e59ac` 二期的设计与实施计划文档,以及 arenacoach 规则目录带真实发生率入账 backlog。
- `79990b3` `b0f0efb` `42bcce1` `b08b6a9` 全仓代码注释中英化。
